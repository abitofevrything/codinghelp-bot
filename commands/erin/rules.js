        ...
        if (message.content.endsWith('-here')) {
            if (allOrNumber === 'all') { // all specified
                ...
                if (nb < 1 || nb > rules.length || isNaN(nb)) { // Gives an error if a correct rule number isn't specified.
                    message.react('❌');
                    message.channel.send({ content: "Please enter a valid rule number. If you aren't sure what is a valid rule number, please check: https://codinghelp.site/all-rules/" });
                    return;
                }

...
const url = 'https://codinghelp.site/link-forums/rules.26/';
const warn = 'https://codinghelp.site/help/rules/#rule-warnban';
...
const rule1 = new Discord.MessageEmbed()
    ...
    .setDescription(`No spam, advertising, or NSFW content. Be Nice & Use common sense. If you are found to post spam or advertise you will be [warned or banned as stated here](${warn}).`);
...
const rule3 = new Discord.MessageEmbed()
    ...
    .setURL(url)
    .setThumbnail(bot.avatar)
    .setDescription(`If you need help with a problem in your code, always provide the raw code in GitHub gist or a similar place. If you aren’t sure what places, you can check [this article](https://codinghelp.site/threads/how-do-i-format-my-code-on-discord-reddit-what-about-via-the-forum.23/).`);
        ...
        if (message.content.endsWith('-here')) {
            if (allOrNumber === 'all') { // all specified
                ...
                if (nb < 1 || nb > rules.length || isNaN(nb)) { // Gives an error if a correct rule number isn't specified.
                    message.react('❌');
                    message.channel.send({ content: `Please enter a valid rule number. If you aren't sure what is a valid rule number, please check: ${url}` });
                    return;
                }
